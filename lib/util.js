 ...
 */
export function uniqueDirs(files) {
  const dirs = new Set();
 ...
 */
export function byShortPath(a, b) {
 ...
 */
export function dirsToCreate(files) {
 ...
 */
export function copyFile(obj, callback) {
  let called = false;
  function done(err) {
    if (!called) {
      called = true;
 ...
 */
export const copy = function (files, base, dest) {
...
export const getUser = function (cwd) {
  return Promise.all([
