...
import {addDeployment, getRepoFromPL} from './warpHelper.js';
import {copy, getUser} from './util.js';
import {ownerOrContributor} from './common.js';
import {uploadFiles} from './dragondeploy.js';
 ...
 */
export function getCacheDir(optPath) {
 ...
 */
export const publish = function publish(basePath, config, callback) {
  ...
  return userPromise.then((user) =>
    getRepo(options)
      ...
      .then(async (git) => {
        ...
        const repo = await getRepoFromPL(repoUrl);
        const deployment = repo.deployments.find(
          (deployment) => deployment.commitOid === commit.oid
        );
        const isOwnerOrContrib = await ownerOrContributor(repo, {
          pushing: true,
        });
        ...
        const txId = await uploadFiles(basePath, files, commit, repo);
        await addDeployment(repo, {
          txId,
