...
exports.publish = function publish(basePath, config, callback) {
  ...
  return userPromise.then((user) =>
    getRepo(options)
      ...
      .then(async (git) => {
        // Return if its the test or deploy is false
        if (process.env.npm_lifecycle_event === 'test' || !options.deploy) {
          if (options.push) {
            common.log('Published\n', {color: 'green'});
          }
          return;
        }
        if (!dragondeploy.hasIndexFile(files)) {
        ...
        if (options.push) {
          common.log('Published and Deployed\n', {color: 'green'});
        } else {
          common.log('Deployed\n', {color: 'green'});
        }
      })
      .then(
        () => {
          if (!options.push && !options.deploy) {
            ...
            common.log('No files pushed or deployed!!!', {color: 'red'});
          }
          done();
        },
        (error) => {
