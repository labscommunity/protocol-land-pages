...
function spawn(exe, args, cwd) {
  return new Promise((resolve, reject) => {
    ...
    child.stderr.on('data', (chunk) => {
      ...
      if (!['log', 'config'].includes(args[0])) {
        console.log(errorMessage);
      }
    });
    child.stdout.on('data', (chunk) => {
      const outputMessage = chunk.toString();
...
Git.prototype.getLatestCommit = function () {
  return this.exec('log', '-1', '--pretty=format:%H,%s')
    .then((git) => {
      const [oid, message] = git.output.split(',');
